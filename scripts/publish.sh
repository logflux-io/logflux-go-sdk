...
# Publish LogFlux Go SDK to a local public repository directory.
# Extracts sdks/go/ from the monorepo, squashes into a single clean commit,
# and syncs to the local public repo.
#
# The public repo lives at ../logflux-public/logflux-go-sdk/.
...
PUBLIC_DIR="$(dirname "$MONO_ROOT")/logflux-public/logflux-go-sdk"
WORK_DIR="$(mktemp -d)"
...
COMMIT_MSG="LogFlux Go SDK"
if [ -n "$TAG" ]; then
    COMMIT_MSG="LogFlux Go SDK $TAG"
fi
...
if [ -d "$PUBLIC_DIR/.git" ]; then
    ...
    git remote add origin git@github.com:logflux-io/logflux-go-sdk.git
fi
